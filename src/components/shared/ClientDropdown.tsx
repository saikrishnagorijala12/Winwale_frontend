...
export const ClientDropdown: React.FC<ClientDropdownProps> = ({
  ...
  const isAllSelected = allowAll && selectedClient === 0;
  ...
  return (
    <div className={`relative group/select ${className}`} ref={dropdownRef}>
      <div
        ...
        className={`w-full pl-4 pr-3.25 ${compact ? "py-3" : "py-4"} text-slate-500 bg-slate-50/50 border ${compact ? "rounded-xl" : "rounded-2xl"} outline-none font-medium flex items-center justify-between transition-all ${
          disabled
      ...
      >
        <div className="flex items-center text-sm gap-3 w-full overflow-hidden">
          <Building2
            className={`w-5 h-5 shrink-0 transition-colors ${
              isOpen ? "text-[#3399cc]" : "text-slate-400"
            }`}
            size={18}
          />
          <span
            className={`block truncate ${
              selectedClientData || isAllSelected
                ? "text-slate-500"
                : "text-slate-500"
            }`}
          >
        ...
        <ChevronDown
          className={`w-4.25 h-4 shrink-0 text-slate-400 transition-transform duration-200 ${
            isOpen ? "rotate-180 text-[#3399cc]" : ""
          }`}
        />
      </div>
