...
interface RunAnalysisStepProps {
    activeClient: ClientMinimal | undefined;
                        ...
                        <div className="relative z-10 flex flex-col items-center animate-analyze">
                            ...
                            <div className="flex flex-col items-center gap-5 w-full max-w-sm">
                                <div className="flex items-center gap-3 text-sm sm:text-base text-slate-600 font-medium">
                                    ...
                                    <span>{message || `Comparing ${totalRows.toLocaleString()} File(s) with GSA contract...`}</span>
                                </div>

                                <div className="w-full h-2.5 bg-slate-100 rounded-full overflow-hidden shadow-inner border border-slate-200/50">
                                    <div 
                                        className={`h-full bg-gradient-to-r from-[#3399cc] via-cyan-400 to-[#3399cc] transition-all duration-500 rounded-full ${percent === 0 ? 'w-full animate-progress' : ''}`} 
                                        style={percent > 0 ? { width: `${percent}%` } : {}}
                                    />
                                </div>
                                <p className="text-xs text-slate-400 font-medium text-center">
                                    {percent > 0 ? `${percent}% Complete` : "Please wait, this might take a moment depending on file size"}
                                </p>
                            </div>
                        </div>
                    </div>
