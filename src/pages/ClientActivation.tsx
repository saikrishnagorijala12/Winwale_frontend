...
const ClientActivation = () => {
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      await clientService.updateClientStatus(client_id, action);
      toast.success(
  ...
  const handleBulkStatusUpdate = async (action: "approve" | "reject") => {
    ...
    try {
      setLoading(true);
      await clientService.bulkUpdateClientStatus(
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-blue-50/30 to-slate-100 p-4 sm:p-6 lg:p-10 space-y-6 sm:space-y-8 lg:space-y-10">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
        {[
          ...
          <div
            key={stat.label}
            className="bg-white p-4 sm:p-6 rounded-xl sm:rounded-3xl shadow-sm flex items-center gap-3 sm:gap-5 border border-slate-100"
          >
            <div
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                clients.map((client) => (
                  <tr
                    key={client.client_id}
                    className={`group transition-colors cursor-pointer ${selectedClientIds.has(client.client_id)
                      ? "bg-blue-50/50"
