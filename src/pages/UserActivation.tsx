...
export default function UserActivation() {
  ...
  const handleBulkStatusUpdate = async (action: "approve" | "reject") => {
    ...
    try {
      ...
      toast.success(
        `${selectedUserIds.size} users ${action === "approve" ? "approved" : "rejected"} successfully`,
      );
      setSelectedUserIds(new Set());
  ...
  const confirmStatusChange = () => {
    if (confirmModal.user) {
      handleApprove(String(confirmModal.user.user_id), confirmModal.action);
    }
  };
                ...
                users.map((user) => (
                  <tr
                    ...
                    className={`group transition-colors ${
                      selectedUserIds.has(String(user.user_id))
                        ? "bg-blue-50/50"
