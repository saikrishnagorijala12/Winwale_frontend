...
// type AuthStep = 'credentials' | 'mfa-sms' | 'mfa-totp' | 'totp-setup-qr' | 'totp-setup-verify';
...
//   // Guard against multiple successful auth executions
//   const isAuthProcessed = useRef(false);

//   useEffect(() => {
//     if (authStep === 'totp-setup-qr' && totpSecret && email) {
//       const issuer = 'Winvale';
//       // URL encode issuer and email to ensure QR validity
...
//     // Always clear code when moving between MFA steps
//     setMfaCode('');
...
//         break;
//       default:
//         setError(`An unexpected authentication state occurred: ${nextStep.signInStep}`);
//         break;
//     }
//     setLoading(false);
//   };
...
//       // Small visual feedback is better than a blocky alert
...
//             <Link to="/forgot-password" title="Recover your password" className="text-sm font-semibold text-blue-600 hover:text-blue-700">
//               Forgot password?
//             </Link>
//           </div>
...
//           >
//             {loading ? <Loader2 className="h-5 w-5 animate-spin" /> : config.buttonText}
//           </button>
//         </form>
...
//             <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Manual Entry Key</label>
//             <div className="flex items-center justify-between gap-3 bg-white p-3 rounded-lg border border-slate-200">
//               <code className="text-sm font-mono text-slate-900 truncate">{totpSecret}</code>
//               <button 
//                 id="copy-btn"
//                 type="button"
//                 onClick={copySecret}
...
//             type="button"
//             onClick={() => {
//               setMfaCode(''); // Ensure input is clean for next step
  ...
  const handleNextStep = async (nextStep: any) => {
    ...
    if (nextStep.signInStep === "DONE") {
      ...
      try {
        ...
        // Let App/AuthContext handle authorization.
        await getCurrentUser(); // optional sanity check
        onAuthSuccess(); // notify App to refresh auth state
      } catch (err) {
        setError("Login successful, but profile could not be loaded.");
      } finally {
        setLoading(false);
